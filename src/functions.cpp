...
// bool IsMouseHoveringObject(GLFWwindow* window, glm::vec3 objPos, glm::mat4
// view, glm::mat4 proj, float boxSize) {
...
//     glm::vec4 rayStart_World = invM * rayStart_NDC; rayStart_World /=
//     rayStart_World.w; glm::vec4 rayEnd_World   = invM * rayEnd_NDC;
//     rayEnd_World /= rayEnd_World.w;
//
//     glm::vec3 rayDir = glm::normalize(glm::vec3(rayEnd_World -
//     rayStart_World)); glm::vec3 rayOrigin = glm::vec3(rayStart_World);
//
...
// }
//
// void imgizmo_hover_translate(GLFWwindow* window, glm::vec3 &objPos, glm::mat4
// viewMatrix, glm::mat4 projectionMatrix, float box_size)
// {
//
//     // Check hover state (0.3f defines the click boundary radius around
//     objPos) bool isHovered = IsMouseHoveringObject(window, objPos,
//     viewMatrix, projectionMatrix, 0.3f); static float timeout = 0.0f; float
//     increase = 1.0f * deltaTime; if (isHovered || ImGuizmo::IsUsing() ||
...
//     }
//
//     // Show the gizmo ONLY if hovering OR if the user is currently dragging
//     it if (timeout < 1.0f) {
...
//
//         if (ImGuizmo::IsUsing()) {
//             objPos = glm::vec3(lightModelMatrix[3]); // Update your actual
...
unsigned int loadTextureFromFile(char const *path) {
  ...
  // 3. Apply it to the texture (usually 4.0f or 8.0f is perfect, or use the
  // max)
  if (Settings::AnistrophicFiltering)
...
unsigned int loadTextureFromColor(glm::vec3 color) {
  ...
  float alpha = 1.0f;
  unsigned char pixelData[] = {static_cast<unsigned char>(color.r * 255.0f),
...
glm::mat4 rotateModel(glm::mat4 &rubixModel, float rotationSpeed,
  ...
  return glm::rotate(rubixModel, (float)glfwGetTime() * 3.0f,
                     glm::vec3(sin(angle), cos(angle), 0.5f));
}

float calc_aspect_ratio(float width, float height) { return width / height; }
