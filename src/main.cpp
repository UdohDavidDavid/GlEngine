...
int main(int argc, char **argv) {
    ...
    // Shader rubixShader("./shaders/rubixVertex.glsl",
    // "./shaders/rubixFragment.glsl"); Shader
    // steelShader("./shaders/steelVertex.glsl",
    // "./shaders/steelFragment.glsl");
    ...
    float x_dirlight_offset = (lightDir.x < 0) ? lightDir.x + dirlight_offset : lightDir.x - dirlight_offset;
    float y_dirlight_offset = (lightDir.y < 0) ? lightDir.y + dirlight_offset : lightDir.y - dirlight_offset;
    float z_dirlight_offset = (lightDir.z < 0) ? lightDir.z + dirlight_offset : lightDir.z - dirlight_offset;
    glm::vec3 dirlightposition(x_dirlight_offset, y_dirlight_offset, z_dirlight_offset);
    ...
    while (!glfwWindowShouldClose(window.getGLFWwindow())) {
        ...
        ImGui::SliderFloat3("Light Direction", &dirlight.direction.x, -15.0f,
                            15.0f);
        ImGui::SliderFloat3("Rubix Position", &rubixMesh.position.x, -15.0f,
                            15.0f);
        ...
        ImGui::SliderFloat("ambience", &Settings::AmbienceIntensity, 0.0f,
                           1.0f);
        ...
        ImGui::SliderFloat("specular", &Settings::SpecularIntensity, 0.0f,
                           1.0f);
        if (ImGui::Button("Default specular"))
          Settings::SpecularIntensity = 1.0f;

        ImGui::SliderFloat("Movement Speed", &Settings::CameraSpeed, 7.5f,
                           12.50f);
        ...
        dirlight_projection = glm::perspective(glm::radians(Settings::FOV), aspectRatio,
                                      0.1f, 1000.0f);
